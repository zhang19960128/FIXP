...
if __name__=='__main__':
    natom = 5;
    workingfolder = './QECAL/';
    pwcommand = "srun -n 48 {0:s}pw.x".format(workingfolder);
    phcommand = "srun -n 48 {0:s}ph.x".format(workingfolder);
    ...
    startPH.obtaindyn(workingfolder + 'bto.dyn');
    DeltaP = np.array([0.0, 0.0, 0.04]);
    ...
    for i in range(ITERMAXTIMES):
        ...
        os.system("{0:s} < ITER{1:d} > {2:s}ITER.out{1:d}".format(pwcommand, i, workingfolder));
        os.system("{0:s} < ITERNOE{1:d} > {2:s}ITERNOE.out{1:d}".format(pwcommand, i, workingfolder));
        os.system("{0:s} < ph.in > {2:s}PHOUT{1:d}".format(phcommand, i, workingfolder));
